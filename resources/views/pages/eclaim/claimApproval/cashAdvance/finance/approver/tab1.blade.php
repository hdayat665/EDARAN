    ...
    <table id="tableactive" class="table table-striped table-bordered align-middle">
        ...
        <tbody>
            ...
            @if ($domain->recommender == Auth::user()->id)
                @foreach ($cas as $ca)
                    @if ($ca->f_recommender == 'recommend' && $ca->f_approver == '')
                        <tr>
                            ...
                            @elseif ($ca->status == 'recommend')
                                <td><span class="badge bg-success" data-toggle="paidc"
                                        title="{{ $ca->status_desc }}">Pending</span></td>
                            ...
                            @elseif ($ca->status == 'active')
                                <td><span class="badge bg-lime" data-toggle="activec" title="{{ $ca->status_desc }}">In
                                        Queue</span></td>
                            @endif
                            <td>
                                <a href="#" data-bs-toggle="dropdown" class="btn btn-primary dropdown-toggle"><i
                                        class="fa fa-cogs"></i> Action <i class="fa fa-caret-down"></i></a>
                                <div class="dropdown-menu">
                                    <a href="/cashAdvanceFapproverDetail/{{ $ca->type }}/{{ $ca->id }}"
                                        id="" data-id="" class="dropdown-item"><i class="fa fa-eye"
                                            aria-hidden="true"></i>
                                        View PO
                                    </a>
