...
class EclaimReportController extends Controller
{
    ...
    public function cashadvanceListingView()
    {
        $data = [];

        return view('pages.report.eclaim.cashadvanceListing', $data);
    }
    ...
    public function cashadvanceReportView(Request $r)
    {
        ...
        if ($input['category'] == '') {
            ...
            $view = 'pages.report.eclaim.filter.cashadvanceReport';
        }
        else if($input['category'] == '1'){
            $report_data = $trs->searchReportAll($input);
            $data['allClaims'] = $report_data['general_claims'];
