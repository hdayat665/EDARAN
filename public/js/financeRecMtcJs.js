...
document.getElementById('finRecMtcPrint').addEventListener('click', printAndShowAllRows);
    ...
    travellingData.then(function(response) {
        ...
        if (table) {
            ...
            for (var i = 0; i < data.length; i++) {
                var rowData = data[i];
                var checkboxHTML = rowData.financerec === 'checked' ?
...
function updateTravelData(id,millage, petrol,tolls, parking) {
    requirejs(["sweetAlert2"], function(swal) {
      ...
      $.ajax({
        type: "POST",
        url: "/updateTravelMtcFinanRec/" + id,
        data: formData,
        dataType: "json",
        processData: false,
        contentType: false,
