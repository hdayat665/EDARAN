...
document.getElementById('admRecPrint').addEventListener('click', printAndShowAllRows);
                ...
                $(tableRow).find("#updateButtonTravel").on("click", function() {
                    ...
                    cells.each(function(index) {
                        ...
                        if (index >= 7 && index <= 10) { // Columns 8, 9, 10 are "Petrol," "Tolls," "Parking"
                            ...
                            if (index === 7) {
                                input.attr("id", "millage").attr("name", "millage");
                            } else if (index === 8) {
...
function updateTravelData(id,millage, petrol,tolls, parking) {
    requirejs(["sweetAlert2"], function(swal) {
      ...
      $.ajax({
        type: "POST",
        url: "/updateTravelMtcAdminRec/" + id,
        data: formData,
        dataType: "json",
        processData: false,
        contentType: false,
